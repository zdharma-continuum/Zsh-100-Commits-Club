...
internal use or for the end user. It is proposed that for the latter (it is
recommended, that for internal use, the runnable be called via the `$0` value
obtained as described above), the plugin shall use a `bin/` subdirectory inside
its main dir. The runnable should be put into the directory with a `+x` access
right assigned.
...
It's a recent addition to the standard and no plugin managers support it yet.
