...
documentation]). This is being done either by a plugin manager or by the plugin
itself (see link:#indicator[5th section] for more information).
...
Note that the unload function should contain `unfunction $0`, to also delete the
function itself.
...
2. Two plugins:
    ...
    - `agkozak/agkozak-zsh-prompt`, is
      link:https://github.com/agkozak/agkozak-zsh-prompt/commit/08b5879a[using]
