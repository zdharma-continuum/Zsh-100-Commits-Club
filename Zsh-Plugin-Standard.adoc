...
4. `/`: for debug functions, i.e: for functions that output debug messages to
   ...
   justifies such allocation. Example function name: `/prompt_zinc_dbg_log`.
5. `@`: for API-like functions, i.e: for functions that are on a boundary to a
   ...
   fixed interface. For example the plugin standard
   link:http://zdharma.org/Zsh-100-Commits-Club/Zsh-Plugin-Standard.html#update-register-call[defines]
   the function `@zsh-plugin-run-on-update`, which is exposing a plugin
   manager's functionality in a well-defined way.
...
.zinc_register_hooks() {
    ...
    +zinc_print "Initialization complete"
}
```
