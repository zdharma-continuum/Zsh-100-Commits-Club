...
```zsh
emulate -L zsh
setopt extendedglob warn_create_global typeset_silent \
...
The `reply` and `REPLY` parameters are being often used to return an array or a
scalar from a function, respectively. Their use is naturally limited to the
functions called from the main function of a plugin – they should not be used to
pass data around e.g.: in between prompts, thus it's natural to localize them in
the main function.
