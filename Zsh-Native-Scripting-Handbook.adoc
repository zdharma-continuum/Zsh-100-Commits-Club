...
A project was needing this to do Zle line-continuation tricks (when you put a
backslash-\ at the end of the line and press enter – it is the
line-continuation that occurs at that moment).

The required functionality is: in given string, count number of apostrophes,
but only the unquoted ones. This means that only apostrophes with null or an
even number of preceding backslashes should be accepted into the count:
...
The answer (i.e. the output) to above presentation and example is: `3` (there
are `3` unquoted apostrophes in the string `$buf`).
