...
from multiple lines of text, something `awk` is often used for. For single line
of text, one can use also `s`-flag:
...
```
declare -a array; array=( a b " c" d ); print ${array[(r)[[:space:]][[:alpha:]]]} ▶  c
```
